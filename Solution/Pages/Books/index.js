$(function () {
	...
	var getFilter = function() {
        return {
            ...
            name: $("#NameFilter").val(),
			type: $("#TypeFilter").val(),
			publishDateMin: $("#PublishDateFilterMin").val(),
			publishDateMax: $("#PublishDateFilterMax").val(),
    ...
    if(abp.auth.isGranted('BookStore.Books.Delete')) {
        dataTableColumns.unshift({
                ...
                render: function (data) {
                    return '<input type="checkbox" class="form-check-input select-row-checkbox" data-id="' + data.book.id + '"/>'
            }
        });
    }
    else {
            ...
            $("#delete-selected-items").click(function () {
                if($("#select-all-items-btn").data("selected") === true) {
                    abp.message.confirm(l("DeleteAllRecords"), function (confirmed) {
                       ...
                       bookService.deleteAll(getFilter())
                           .then(function () {
                               dataTable.ajax.reloadEx();
                               selectOrUnselectAllCheckboxes(false);
                               showOrHideContextMenu();
                    ...
                    abp.message.confirm(l("DeleteSelectedRecords", selectedCheckboxes.length), function (confirmed) {
                        ...
                        bookService.deleteByIds(selectedRecordsIds)
                            .then(function () {
                                dataTable.ajax.reloadEx();
                                selectOrUnselectAllCheckboxes(false);
                                showOrHideContextMenu();
